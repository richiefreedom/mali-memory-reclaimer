...
static irqreturn_t sprd_eic_irq_handle(int irq, void *dev_id)
{
	...
	vol_down_keys_pressed = button_state_current;
#if CONFIG_SEC_DEBUG
	if (KEY_VOLUMEDOWN == button->code)
		sec_debug_check_crash_key(button->code, button_state_current);
#endif
